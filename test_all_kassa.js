// Тестовый скрипт для проверки функции extractProductsFromKassaText со всеми файлами
const fs = require('fs');
...
const folderPath = path.join(__dirname, 'ПРИМЕР кассы');
...
fs.readdir(folderPath, (err, files) => {
    ...
    console.log(`Найдено ${textFiles.length} текстовых файлов для тестирования.`);
    
    // Обрабатываем каждый файл
    textFiles.forEach(file => {
        const filePath = path.join(folderPath, file);
        
        console.log(`\n========= ТЕСТИРОВАНИЕ ФАЙЛА: ${file} =========`);
        ...
        if (products.length > 0) {
            ...
            products.slice(0, Math.min(5, products.length)).forEach(product => {
                console.log(`- ${product.name} (${product.quantity} шт. по ${product.price} руб.)`);
            });
            
            // Если товаров больше 5, показываем сообщение
